    ...
    for text_file in os.listdir(text_dir):
        with open(os.path.join(text_dir, text_file), 'r') as f:
            ...
            if class_name == "notwaza" and class_counts["notwaza"] >= 10000:
                continue
            elif class_name == "notwaza" and class_counts["notwaza"] < 10000:
                class_counts[class_name] += 1
                output_text_path = os.path.join(output_label_dir, class_name + "_" + str(class_counts[class_name]).zfill(5))
                ...
                image_names_list.append(class_name + "_" + str(class_counts[class_name]).zfill(5) + ".png")
                ...
                if class_name not in class_counts:
                    ...
                    class_counts[class_name] += 1
                output_text_path = os.path.join(output_label_dir, class_name + "_" + text_file)
                with open(output_text_path, 'w') as f:
                    f.write(first_line)

                output_image_path = os.path.join(output_image_dir, class_name + "_" + text_file.replace('.txt', '.png'))
                # 元画像を読み込む
                original_img = Image.open(os.path.join(input_image_dir[i], text_file.replace('.txt', '.jpeg')))
                # 新しい画像を作成 (608x608, 白背景)
                new_img = Image.new('RGB', (608, 608), 'white')
                ...
                # 保存
                new_img.save(output_image_path, 'PNG')
                image_names_list.append(class_name + "_" + text_file.replace('.txt', '.png'))
